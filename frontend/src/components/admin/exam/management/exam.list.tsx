...
const ExamList: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await deleteExam(deleteDialog.examId);
      if (response.success) {
  ...
  const handleToggleActive = async () => {
    ...
    try {
      const response = await toggleActiveExam(toggleDialog.examId);
      if (response.success) {
        alert(
      ...
      <Card className="shadow-xl border border-gray-200">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Button
              onClick={() => navigate("/admin/content/sw/writing/new")}
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center justify-center gap-2">
                          <IconButton
                            variant="text"
                            size="sm"
                            onClick={() => navigate(`/admin/content/sw/writing/${exam.id}`)}
                            title="Chỉnh sửa"
                          >
